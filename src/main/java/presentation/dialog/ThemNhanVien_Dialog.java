...
import dao.iml.TaiKhoanDaoImpl; // Cần dùng để kiểm tra tên đăng nhập
...
public class ThemNhanVien_Dialog extends JDialog {
    ...
    private TaiKhoanDaoImpl TaiKhoanDaoImpl; // Thêm DAO để kiểm tra

    public ThemNhanVien_Dialog(Frame owner) {
        super(owner, "Thêm nhân viên", true);
        this.TaiKhoanDaoImpl = new TaiKhoanDaoImpl(); // Khởi tạo DAO
        initialize();
    }
    ...
    private void onThemButtonClick() {
        try {
            ...
            if (TaiKhoanDaoImpl.kiemTraTenDangNhapTonTai(tenDangNhap)) {
