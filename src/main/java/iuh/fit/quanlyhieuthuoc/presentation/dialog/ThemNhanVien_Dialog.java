...
import iuh.fit.quanlyhieuthuoc.infrastructure.persistence.TaiKhoanRepositoryImpl; // Cần dùng để kiểm tra tên đăng nhập
...
public class ThemNhanVien_Dialog extends JDialog {
    ...
    private TaiKhoanRepositoryImpl TaiKhoanRepositoryImpl; // Thêm DAO để kiểm tra

    public ThemNhanVien_Dialog(Frame owner) {
        super(owner, "Thêm nhân viên", true);
        this.TaiKhoanRepositoryImpl = new TaiKhoanRepositoryImpl(); // Khởi tạo DAO
        initialize();
    }
    ...
    private void onThemButtonClick() {
        try {
            ...
            if (TaiKhoanRepositoryImpl.kiemTraTenDangNhapTonTai(tenDangNhap)) {
            ...
            NhanVienRepositoryImpl nvDAO = new NhanVienRepositoryImpl();
            TaiKhoanRepositoryImpl tkDAO = new TaiKhoanRepositoryImpl();
