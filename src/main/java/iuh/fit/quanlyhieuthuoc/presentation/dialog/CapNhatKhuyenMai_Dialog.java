...
public class CapNhatKhuyenMai_Dialog extends JDialog {
	...
	private void populateData() {
		...
		dateBatDau.setDate(
				Date.from(khuyenMaiCanCapNhat.getNgayBatDau().atStartOfDay(ZoneId.systemDefault()).toInstant()));
		dateKetThuc.setDate(
				Date.from(khuyenMaiCanCapNhat.getNgayKetThuc().atStartOfDay(ZoneId.systemDefault()).toInstant()));
...
//            txtSoLuongTangThem.setText(String.valueOf(khuyenMaiCanCapNhat.getSoLuongTangThem()));
	...
	private void onLuuButtonClick() {
		...
		try {
			...
			khuyenMaiCanCapNhat
					.setNgayBatDau(dateBatDau.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
			khuyenMaiCanCapNhat
					.setNgayKetThuc(dateKetThuc.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
