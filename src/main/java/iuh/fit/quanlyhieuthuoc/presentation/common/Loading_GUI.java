package iuh.fit.quanlyhieuthuoc.presentation.common;
...
import iuh.fit.quanlyhieuthuoc.infrastructure.persistence.DonViTinhRepositoryImpl;
import iuh.fit.quanlyhieuthuoc.infrastructure.persistence.KhachHangRepositoryImpl;
import iuh.fit.quanlyhieuthuoc.infrastructure.persistence.NhaCungCapRepositoryImpl;
...
import iuh.fit.quanlyhieuthuoc.infrastructure.db.DataSeeder;
import iuh.fit.quanlyhieuthuoc.infrastructure.db.JPAUtil;
import iuh.fit.quanlyhieuthuoc.presentation.DangNhap_GUI;
        ...
        SwingWorker<Boolean, LoadingProgress> worker = new SwingWorker<Boolean, LoadingProgress>() {
            ...
            protected Boolean doInBackground() throws Exception {
                try {
                    ...
                    DonViTinhRepository dvtRepository = new DonViTinhRepositoryImpl();
                    int countDVT = dvtRepository.layTatCaDonViTinh().size();
                    publish(new LoadingProgress(28, "✓ Đã tải " + countDVT + " đơn vị tính"));
                    Thread.sleep(100);
                    
                    publish(new LoadingProgress(30, "Đang tải quy cách đóng gói..."));
                    QuyCachDongGoiRepository qcRepository = new QuyCachDongGoiRepositoryImpl();
                    int countQC = qcRepository.layTatCaQuyCachDongGoi().size();
                    ...
                    NhanVienRepository nvRepository = new NhanVienRepositoryImpl();
                    int countNV = nvRepository.layTatCaNhanVien().size();
                    ...
                    KhachHangRepository khRepository = new KhachHangRepositoryImpl();
                    int countKH = khRepository.layTatCaKhachHang().size();
                    ...
                    NhaCungCapRepository nccRepository = new NhaCungCapRepositoryImpl();
                    int countNCC = nccRepository.layTatCaNhaCungCap().size();
                    ...
                    SanPhamRepository spRepository = new SanPhamRepositoryImpl();
                    int countSP = spRepository.layTatCaSanPham().size();
