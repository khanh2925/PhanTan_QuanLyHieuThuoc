...
public class PhieuTraRepositoryImpl
        extends AbstractGenericRepositoryImpl<PhieuTra, String>
        implements PhieuTraRepository {

    public PhieuTraRepositoryImpl() {
            ...
            return doInTransaction(em -> {
                ...
                if (trangThaiMoi == 2 && trangThaiCu != 2) {
                    ...
                    List<Object> maPHList = em.createNativeQuery(
                            "SELECT TOP 1 ph.MaPhieuHuy FROM PhieuHuy ph " +
                            "INNER JOIN ChiTietPhieuHuy ctph ON ph.MaPhieuHuy = ctph.MaPhieuHuy " +
                            "WHERE ctph.LyDoChiTiet LIKE ? ORDER BY ph.MaPhieuHuy DESC")
                    ...
                    if (!maPHList.isEmpty() && maPHList.get(0) != null) {
                        ...
                        em.createNativeQuery(
                                ...
                                "(SELECT ISNULL(SUM(ThanhTien),0) FROM ChiTietPhieuHuy WHERE MaPhieuHuy=?) " +
                                "WHERE MaPhieuHuy=?")
