...
public class TopSanPhamBanChay_Panel extends JPanel {
    ...
    private void loadDuLieuThongKe() {
        ...
        // Lấy dữ liệu từ DAO
        List<Object[]> danhSach = svc.getTopSanPhamBanChay(tuNgay, denNgay, topN);
        double tongDoanhThuToanBo = svc.getTongDoanhThuTheoKhoangNgay(tuNgay, denNgay);
        ...
        double doanhThuKyTruoc = svc.getTongDoanhThuKyTruoc(tuNgayKyTruoc, denNgayKyTruoc);
        ...
        for (int i = 0; i < danhSach.size(); i++) {
            ...
            double soLuongKyTruoc = svc.getSoLuongBanKyTruoc(maSP, tuNgayKyTruoc, denNgayKyTruoc);
            String trend;
            if (soLuongKyTruoc == 0) {
                if (soLuong > 0) {
                    trend = "↑ Mới";
