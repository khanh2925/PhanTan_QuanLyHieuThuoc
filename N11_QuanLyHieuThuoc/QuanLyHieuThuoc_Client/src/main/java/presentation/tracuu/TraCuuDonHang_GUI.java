...
//        txtTimKiem.addActionListener(ev -> xuLyTimKiem());
    ...
    private void xuLyLamMoi() {
        try {
            allHoaDon = clientService.getAllHoaDon();
        } catch (Exception e) {
            allHoaDon = new ArrayList<>();
            JOptionPane.showMessageDialog(this, "Lỗi tải danh sách hóa đơn: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
        txtTimKiem.setText("");
        SwingUtilities.invokeLater(() -> {
            txtTimKiem.requestFocusInWindow();
    ...
    private void xuLyTimKiem() {
        ...
        if (!tuKhoa.isEmpty()) {
            if (tuKhoa.toUpperCase().startsWith("HD-")) {
                HoaDon hd = null;
                try {
                    hd = clientService.getHoaDonByCode(tuKhoa);
                } catch (Exception ignored) {
                }
                if (hd != null) ketQua.add(hd);
            } else {
                try {
                    ketQua = clientService.searchHoaDonByPhone(tuKhoa);
                } catch (Exception e) {
    ...
    private void loadChiTietTuDongChon() {
        ...
        if (row >= 0) {
            ...
            if (hdChon == null) {
                try {
                    hdChon = clientService.getHoaDonByCode(maHD);
                } catch (Exception ignored) {
                }
            }
    ...
    private void xemLaiHoaDon(String maHD) {
        HoaDon hd = null;
        try {
            hd = clientService.getHoaDonByCode(maHD);
        } catch (Exception ignored) {
        }
        if (hd != null) {
