...
public class KhachHang_NV_GUI extends JPanel implements ActionListener, DocumentListener, KeyListener {
    ...
    private boolean validData() {
        ...
        if (ngaySinhDate == null) {
            JOptionPane.showMessageDialog(this, "Ngày sinh không được để trống và khách hàng phải từ 16 tuổi trở lên!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        java.time.LocalDate ngaySinh = new java.sql.Date(ngaySinhDate.getTime()).toLocalDate();
        ...
        if (ngaySinh.isAfter(java.time.LocalDate.now().minusYears(16))) {
            JOptionPane.showMessageDialog(this, "Khách hàng phải từ 16 tuổi trở lên!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        return true;
    }
