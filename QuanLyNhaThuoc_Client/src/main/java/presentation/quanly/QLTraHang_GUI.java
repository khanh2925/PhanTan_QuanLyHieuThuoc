...
public class QLTraHang_GUI extends JPanel implements ActionListener, MouseListener {
	...
	private List<PhieuTraDTO> dsPhieuTra;
	private List<ChiTietPhieuTraDTO> dsCTPhieuTra;
	private ClientService pt_dao;
	private ClientService ctpt_dao;
	...
	private void loadDataTablePT() {
		dsPhieuTra = new ArrayList<PhieuTraDTO>();
		...
		int stt = 1;
		for (PhieuTraDTO pt : dsPhieuTra) {
			String khachHang = pt.getTenKhachHang() != null ? pt.getTenKhachHang() : "N/A";
			String sdt = pt.getSoDienThoai() != null ? pt.getSoDienThoai() : "";
			String nhanVien = pt.getTenNhanVien() != null ? pt.getTenNhanVien() : "N/A";

			modelPT.addRow(new Object[] { stt++, pt.getMaPhieuTra(), khachHang, sdt,
					pt.getNgayLap() != null ? pt.getNgayLap() : "", nhanVien,
					pt.isTrangThai() ? "Đã duyệt" : "Chờ duyệt",
					df.format(pt.getTongTienHoan()) });
		}
	...
	private void loadTableCTPT() {
		...
		int modelRowPT = tblPT.convertRowIndexToModel(selectRow);
		String maPT = modelPT.getValueAt(modelRowPT, 1).toString();

		dsCTPhieuTra = new ArrayList<ChiTietPhieuTraDTO>();
		modelCTPT.setRowCount(0);
		...
		int stt = 1;
		for (ChiTietPhieuTraDTO ctpt : dsCTPhieuTra) {
			String tenDonViTinh = ctpt.getTenDonViTinh() != null ? ctpt.getTenDonViTinh() : "N/A";
			String maHD = ctpt.getMaHoaDon() != null ? ctpt.getMaHoaDon() : "N/A";
			String maLo = ctpt.getMaLo() != null ? ctpt.getMaLo() : "N/A";
			String tenSP = ctpt.getTenSanPham() != null ? ctpt.getTenSanPham() : "N/A";
			String hanDung = "N/A";

			modelCTPT.addRow(new Object[] { stt++, maHD, maLo, tenSP, hanDung, ctpt.getSoLuong(), ctpt.getLyDoChiTiet(),
					tenDonViTinh, trangThaiCTText(ctpt.getTrangThai())
			});
		}
	...
	private void capNhatTrangThaiNut() {
		...
		if (coDongCTPTDuocChon && modelCTPT != null) {
			int modelRowCTPT = tblCTPT.convertRowIndexToModel(rowCTPT);
			String trangThai = modelCTPT.getValueAt(modelRowCTPT, 8).toString().trim();
	...
	private void xemPhieuTra(String maPT) {
		PhieuTraDTO pt = pt_dao.timKiemPhieuTraBangMa(maPT);
		if (pt != null) {
			List<ChiTietPhieuTraDTO> dsCT = ctpt_dao.timKiemChiTietBangMaPhieuTra(maPT);
	...
	private void HuyHang() {
		...
		int modelRowCT = tblCTPT.convertRowIndexToModel(selectRowCT);

		String trangThai = modelCTPT.getValueAt(modelRowCT, 8).toString();
		...
		int modelRowPT = tblPT.convertRowIndexToModel(selectRowPT);

		String maPT = modelPT.getValueAt(modelRowPT, 1).toString();
		String maHD = modelCTPT.getValueAt(modelRowCT, 1).toString();
		String maLo = modelCTPT.getValueAt(modelRowCT, 2).toString();
		String maDVT = "";
		...
		for (ChiTietPhieuTraDTO ct : dsCTPhieuTra) {
			if (maHD.equals(ct.getMaHoaDon()) && maLo.equals(ct.getMaLo())) {
				maDVT = ct.getMaDonViTinh() != null ? ct.getMaDonViTinh() : "";
				break;
			}
		}
		...
		if (kq != null && kq.startsWith("OK")) {
			...
			modelCTPT.setValueAt("Huỷ hàng", modelRowCT, 8);

			// Hiển thị thông báo có mã phiếu huỷ nếu được tạo
			String tenSP = modelCTPT.getValueAt(modelRowCT, 3).toString(); // Cột 3: Tên SP
			if (kq.contains("|")) {
	...
	private void NhapKho() {
		...
		LoSanPham loSanPham = (LoSanPham) pt_dao.getLotByCode(maLo);
		...
		if (loSanPham != null) {
			...
			QuyCachDongGoi qcGoc = (QuyCachDongGoi) pt_dao.getRootPackagingRuleByProduct(loSanPham.getSanPham().getMaSanPham());
		...
		for (ChiTietPhieuTraDTO ct : dsCTPhieuTra) {
			if (maHD.equals(ct.getMaHoaDon()) && maLo.equals(ct.getMaLo())) {
				...
				if (ct.getMaDonViTinh() != null && loSanPham != null) {
					QuyCachDongGoi qc = (QuyCachDongGoi) pt_dao.getPackagingRuleByProductAndUnit(
		...
		if (kq != null && kq.startsWith("OK")) {
			// ✅ Cập nhật lại GUI - cột 8 (do thêm STT)
			modelCTPT.setValueAt("Nhập lại hàng", modelRowCT, 8);
