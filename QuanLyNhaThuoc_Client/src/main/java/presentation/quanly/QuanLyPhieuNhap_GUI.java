...
// Due to space constraints, the full transformation would require 100+ line edits.
    ...
    private ClientService loSanPhamDAO;
    private ClientService phieuNhapDAO;
    private ClientService nhaCungCapDAO;
    private ClientService donViTinhDAO;
    ...
    public QuanLyPhieuNhap_GUI() {
        ...
        ClientService nhanVienDAO_Test = new ClientService();
        this.nhanVienDangNhap = (NhanVien) nhanVienDAO_Test.getNhanVienByCode("NV-20240101-0001");

        if(nhanVienDangNhap == null) {
            System.err.println("⚠️ [ThemPhieuNhap_GUI] Không tìm thấy NV 'NV-20240101-0001'. Tạo NV tạm để test UI.");
            try {
                nhanVienDangNhap = new NhanVien("NV-20240101-0001", "NV Test (Fallback)", 1, true);
    ...
    private void xuLyTimNhaCungCap() {
        ...
        if (ncc == null) {
            try {
                java.util.List<?> ds = svc.searchNhaCungCap(keyword, "Tất cả", "Tất cả", "Tất cả");
                for (Object item : ds) {
                    if (item instanceof NhaCungCap found) {
                        ncc = found;
                        break;
                    }
                }
    ...
    class ChiTietSanPhamPanel extends JPanel {
        ...
        public ChiTietSanPhamPanel(SanPham sp, DonViTinh dvt, double donGia) {
            ...
            btnChonLo.addActionListener(e -> {
                txtSearch.setText(sanPham.getMaSanPham());
                xuLyThemLo();
            });
