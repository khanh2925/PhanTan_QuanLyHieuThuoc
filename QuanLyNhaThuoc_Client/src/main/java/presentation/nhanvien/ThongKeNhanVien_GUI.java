...
public class ThongKeNhanVien_GUI extends JPanel implements ActionListener, MouseListener {
    ...
    private void updateStatsFromService(Date tu, Date den, int caLam) {
        ...
        if (remote != null) {
            tongDoanhSo = getDoubleValue(remote, "tongDoanhSo", 0);
            soHoaDon = getIntValue(remote, "soHoaDon", 0);
            tongTienTra = getDoubleValue(remote, "tongTienTra", 0);
            soPhieuTra = getIntValue(remote, "soPhieuTra", 0);
            soPhieuHuy = getIntValue(remote, "soPhieuHuy", 0);
            giaTriTrungBinh = getDoubleValue(remote, "giaTriTrungBinh", soHoaDon > 0 ? tongDoanhSo / soHoaDon : 0);
            tyLeHoanTra = getDoubleValue(remote, "tyLeHoanTra", soHoaDon > 0 ? (soPhieuTra * 100.0 / soHoaDon) : 0);
            return;
        }
        tongDoanhSo = 0;
        soHoaDon = 0;
    ...
    private double getDoubleValue(Object source, String name, double fallback) {
        Object value = getValue(source, name);
        return value != null ? getDouble(value) : fallback;
    }

    private int getIntValue(Object source, String name, int fallback) {
        Object value = getValue(source, name);
        return value != null ? getInt(value) : fallback;
    }

    private Object getValue(Object source, String name) {
        try {
            java.lang.reflect.Field f = source.getClass().getDeclaredField(name);
