...
public class HuyHangNhanVien_GUI extends JPanel implements ActionListener {
	...
	private void importDanhSachLo(ArrayList<LoSanPham> danhSachLo, String loaiTim) {
		...
		for (LoSanPham lo : danhSachLo) {
			if (lo == null || lo.getSanPham() == null || lo.getSoLuongTon() <= 0) {
				continue;
			}
			double giaNhap = lo.getSanPham().getGiaNhap();
			if ("HSD".equals(loaiTim)) {
				addDongHuyVoiLyDo(lo, giaNhap, PLACEHOLDER_HET_HAN);
			} else {
				addDongHuy(lo, giaNhap);
			}
		}
	...
	private void xuLyTaoPhieuHuy() {
		...
		if (tk == null || tk.getMaNhanVien() == null) {
			JOptionPane.showMessageDialog(this, "Không xác định được nhân viên lập phiếu huỷ.", "Lỗi",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		NhanVien nv = new NhanVien(tk.getMaNhanVien(), tk.getTenNhanVien(), "Quản lý".equals(tk.getVaiTro()), 1);
		...
		new PhieuHuyPreviewDialog(w, toPhieuHuyDTO(ph)).setVisible(true);
	...
	private PhieuHuyDTO toPhieuHuyDTO(PhieuHuy ph) {
		...
		dto.setTongTien(ph.getTongTien());
		if (ph.getNhanVien() != null) {
			NhanVienDTO nv = new NhanVienDTO();
			nv.setMaNhanVien(ph.getNhanVien().getMaNhanVien());
			nv.setTenNhanVien(ph.getNhanVien().getTenNhanVien());
			nv.setSoDienThoai(ph.getNhanVien().getSoDienThoai());
			dto.setNhanVien(nv);
		}
		List<ChiTietPhieuHuyDTO> chiTiet = new ArrayList<>();
		if (ph.getChiTietPhieuHuyList() != null) {
			for (ChiTietPhieuHuy ct : ph.getChiTietPhieuHuyList()) {
				...
				if (ct.getLoSanPham() != null) {
					LoSanPhamDTO loDto = new LoSanPhamDTO();
					loDto.setMaLo(ct.getLoSanPham().getMaLo());
					if (ct.getLoSanPham().getSanPham() != null) {
						loDto.setMaSanPham(ct.getLoSanPham().getSanPham().getMaSanPham());
						loDto.setTenSanPham(ct.getLoSanPham().getSanPham().getTenSanPham());
					}
					ctDto.setMaLo(loDto.getMaLo());
					ctDto.setLoSanPham(loDto);
				}
				if (ct.getDonViTinh() != null) {
					DonViTinhDTO dvtDto = new DonViTinhDTO();
					dvtDto.setMaDonViTinh(ct.getDonViTinh().getMaDonViTinh());
					dvtDto.setTenDonViTinh(ct.getDonViTinh().getTenDonViTinh());
					ctDto.setDonViTinh(dvtDto);
				}
		...
		dto.setChiTietPhieuHuyList(chiTiet);
		return dto;
	}
