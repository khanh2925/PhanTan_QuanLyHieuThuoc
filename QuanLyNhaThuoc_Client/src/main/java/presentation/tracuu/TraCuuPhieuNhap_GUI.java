...
public class TraCuuPhieuNhap_GUI extends JPanel implements ActionListener, MouseListener {
    ...
    private void setupKeyboardShortcuts() {
        ...
        actionMap.put("xemHoaDon", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                PhieuNhapDTO phieuNhap = layPhieuNhapTheoMa(maPhieuNhap);
                if (phieuNhap != null) {
        ...
        if (e.getSource().equals(tblPhieuNhap)) {
            if (e.getClickCount() == 2) {
                ...
                if (row != -1) {
                    ...
                    PhieuNhapDTO pn = layPhieuNhapTheoMa(maPN);
                    if (pn != null) {
                        hienThiHoaDon(pn);
                    }
                }
    ...
    private List<PhieuNhapDTO> layDanhSachPhieuNhap() {
        try {
    ...
    private PhieuNhapDTO layPhieuNhapTheoMa(String maPhieuNhap) {
        try {
            Object o = svc.getPhieuNhapByCode(maPhieuNhap);
            if (o instanceof PhieuNhapDTO) {
                return (PhieuNhapDTO) o;
            }
        } catch (Exception ex) {
        }
        return null;
    ...
    private LocalDate parseNgayNhap(String ngayNhap) {
        if (ngayNhap == null || ngayNhap.isBlank()) {
            return null;
        }
        try {
            return LocalDate.parse(ngayNhap, fmt);
        } catch (Exception ex) {
            return null;
        }
    }
    ...
    private void taiDuLieuPhieuNhap() {
        ...
        List<PhieuNhapDTO> listPN = layDanhSachPhieuNhap();
        ...
        for (PhieuNhapDTO pn : listPN) {
            modelPhieuNhap.addRow(new Object[] {
                    stt++,
                    pn.getMaPhieuNhap(),
                    pn.getNgayNhap() != null ? pn.getNgayNhap() : "",
                    pn.getTenNhanVien() != null ? pn.getTenNhanVien() : "",
                    pn.getTenNhaCungCap() != null ? pn.getTenNhaCungCap() : "",
    ...
    private void xuLyTimKiem() {
        ...
        if (!listPN.isEmpty()) {
            ...
            List<PhieuNhapDTO> filtered = new ArrayList<>();
            for (PhieuNhapDTO pn : listPN) {
                LocalDate ngayNhap = parseNgayNhap(pn.getNgayNhap());
                boolean inRange = ngayNhap != null && !ngayNhap.isBefore(tu) && !ngayNhap.isAfter(den);
                if (!inRange) continue;
                if (!kw.isEmpty()) {
                    String ma = pn.getMaPhieuNhap() != null ? pn.getMaPhieuNhap().toLowerCase() : "";
                    String nv = pn.getTenNhanVien() != null ? pn.getTenNhanVien().toLowerCase() : "";
                    String ncc = pn.getTenNhaCungCap() != null ? pn.getTenNhaCungCap().toLowerCase() : "";
        ...
        if (listPN == null || listPN.isEmpty()) listPN = new ArrayList<>();
    ...
    private void hienThiChiTietPhieuNhap(String maPN) {
        ...
        if (pn != null && pn.getChiTietList() != null) {
            int stt = 1;
            for (ChiTietPhieuNhapDTO ct : pn.getChiTietList()) {
                modelChiTiet.addRow(new Object[] {
                        stt++,
                        ct.getMaLo() != null ? ct.getMaLo() : "",
                        ct.getMaSanPham() != null ? ct.getMaSanPham() : "",
                        ct.getTenSanPham() != null ? ct.getTenSanPham() : "Không xác định",
                        ct.getTenDonViTinh() != null ? ct.getTenDonViTinh() : "",
                        ct.getSoLuongNhap(),
     ...
     */
    private void hienThiHoaDon(PhieuNhapDTO phieuNhap) {
        ...
        pnHeader.add(taoNhanInDam(phieuNhap.getTenNhanVien() != null ? phieuNhap.getTenNhanVien() : "", labelFont));

        pnHeader.add(taoNhanThuong("Ngày lập phiếu:", labelFont));
        pnHeader.add(taoNhanInDam(phieuNhap.getNgayNhap() != null ? phieuNhap.getNgayNhap() : "", labelFont));

        pnHeader.add(taoNhanThuong("Nhà cung cấp:", labelFont));
        pnHeader.add(taoNhanInDam(phieuNhap.getTenNhaCungCap() != null ? phieuNhap.getTenNhaCungCap() : "", labelFont));

        pnHeader.add(taoNhanThuong("Mã nhà cung cấp:", labelFont));
        pnHeader.add(taoNhanInDam(phieuNhap.getMaNhaCungCap() != null ? phieuNhap.getMaNhaCungCap() : "", labelFont));
        ...
        if (phieuNhap.getChiTietList() != null) {
            for (ChiTietPhieuNhapDTO ct : phieuNhap.getChiTietList()) {
