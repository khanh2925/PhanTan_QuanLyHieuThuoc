...
public class TraCuuKhachHang_GUI extends JPanel {
    ...
    // DAO
    private final ClientService khachHangDAO;
    private final ClientService hoaDonDAO;
    private final ClientService phieuTraDAO;

    // Data cache
    private List<KhachHangDTO> danhSachGoc = new ArrayList<>();
    ...
    public TraCuuKhachHang_GUI() {
        ...
        khachHangDAO = new ClientService();
        hoaDonDAO = new ClientService();
        phieuTraDAO = new ClientService();
    ...
    private void timKiem() {
        ...
        if (keyword.isEmpty() || keyword.equals(PLACEHOLDER_TIM_KIEM)) {
            ...
            if (danhSachGoc == null || danhSachGoc.isEmpty()) {
                danhSachGoc = layTatCaKhachHangDTO();
            }
        ...
        // 2. Nếu có keyword cụ thể → Tìm kiếm trong cache
            ...
            // Tìm kiếm trên cache
            for (KhachHangDTO kh : danhSachGoc) {
        ...
        if (!"Tất cả".equals(gioiTinh)) {
            ketQua.removeIf(kh -> !gioiTinh.equals(kh.getGioiTinh()));
        }

        // --- Áp dụng bộ lọc: trạng thái ---
        if (!"Tất cả".equals(trangThai) && !"Hoạt động".equals(trangThai)) {
            ketQua.clear();
        }
     ...
     */
    private void loadKetQuaTimKiem(List<KhachHangDTO> dsKhachHang) {
        modelKhachHang.setRowCount(0);
        int stt = 1;
        for (KhachHangDTO kh : dsKhachHang) {
            String gioiTinh = kh.getGioiTinh() != null ? kh.getGioiTinh() : "";
            String ngaySinh = kh.getNgaySinh() != null ? kh.getNgaySinh() : "";
    ...
    private List<KhachHangDTO> layTatCaKhachHangDTO() {
        List<KhachHangDTO> result = new ArrayList<>();
        List<?> ds = khachHangDAO.getAllKhachHang();
        if (ds != null) {
            for (Object item : ds) {
                KhachHangDTO dto = toKhachHangDTO(item);
                if (dto != null) {
                    result.add(dto);
                }
            }
    ...
    private KhachHangDTO toKhachHangDTO(Object item) {
        if (item instanceof KhachHangDTO dto) {
            return dto;
        }
        if (item instanceof KhachHang kh) {
            ...
            dto.setNgaySinh(kh.getNgaySinh() != null ? kh.getNgaySinh().format(dtf) : null);
            return dto;
        }
        return null;
    }
    ...
    private void loadLichSuGiaoDich(String maKH) {
        ...
        try {
            ...
            KhachHangDTO khachHang = danhSachGoc.stream()
                    .filter(kh -> maKH.equals(kh.getMaKhachHang()))
                    .findFirst()
                    .orElse(null);
            ...
            for (HoaDonDTO hd : allHoaDon) {
                if (sdtKhachHang != null && sdtKhachHang.equals(hd.getSdtKhachHang())) {
                    String ngayLap = hd.getNgayLap() != null ? hd.getNgayLap() : "";
                    String nhanVien = hd.getTenNhanVien() != null ? hd.getTenNhanVien() : "";
            ...
            List<PhieuTraDTO> allPhieuTra = phieuTraDAO.getAllPhieuTra();
            int stt2 = 1;
            for (PhieuTraDTO pt : allPhieuTra) {
                if (maKH.equals(pt.getMaKhachHang())) {
                    String ngayLap = pt.getNgayLap() != null ? pt.getNgayLap() : "";

                    String lyDo = "";
                    List<ChiTietPhieuTraDTO> dsChiTiet = phieuTraDAO.getChiTietPhieuTraByMa(pt.getMaPhieuTra());
                    if (dsChiTiet != null && !dsChiTiet.isEmpty()) {
                        lyDo = dsChiTiet.get(0).getLyDoChiTiet();
                        if (dsChiTiet.size() > 1) {
                            lyDo += " (+" + (dsChiTiet.size() - 1) + " SP khác)";
                        }
                    }
