...
public class TraCuuNhaCungCap_GUI extends JPanel implements ActionListener {
	...
	private final ClientService nhaCungCapDAO;
	private final ClientService phieuNhapDAO;
	private final ClientService sanPhamDAO;
	private List<NhaCungCapDTO> allNhaCungCap = new ArrayList<>();
	...
	public TraCuuNhaCungCap_GUI() {
		...
		nhaCungCapDAO = new ClientService();
		phieuNhapDAO = new ClientService();
	...
	private void taiDanhSachNhaCungCap() {
		allNhaCungCap = nhaCungCapDAO.getAllNhaCungCapDTO();
	}
	...
	private void xuLyTimKiem() {
		...
		List<NhaCungCapDTO> ketQua = new ArrayList<>();
	...
	// ==============================================================================
	private void loadTableNhaCungCap(List<NhaCungCapDTO> ds) {
	...
	private void loadLichSuNhapHang(String maNCC) {
		...
		try {
			List<PhieuNhapDTO> dsPhieuNhap = phieuNhapDAO.getPhieuNhapByNhaCungCap(maNCC);

			int stt = 1;
			for (PhieuNhapDTO pn : dsPhieuNhap) {
				modelLichSu.addRow(new Object[] { stt++, pn.getMaPhieuNhap(), pn.getNgayNhap() != null ? pn.getNgayNhap() : "",
						df.format(pn.getTongTien()), pn.getTenNhanVien() != null ? pn.getTenNhanVien() : "" });
			}
		} catch (Exception e) {
	...
	private void loadSanPhamCungCap(String maNCC) {
		...
		for (Map.Entry<String, Object[]> entry : thongKe.entrySet()) {
			...
			modelSanPham.addRow(new Object[] { stt++, maSP, data[0], // Tên SP
					data[1] != null ? data[1].toString() : "", // Loại
					data[2], // Số lần nhập
					data[3] // Tổng SL
			});
		}
	...
	private void xuatExcel() {
		...
		List<NhaCungCapDTO> danhSachCanXuat;
		String tenFile;
		...
		if (coChonDong) {
			...
			for (int row : selectedRows) {
				String maNCC = tblNhaCungCap.getValueAt(row, 1).toString();
				NhaCungCapDTO ncc = allNhaCungCap.stream()
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook()) {
			...
			int rowIdx = 1;
			for (NhaCungCapDTO ncc : danhSachCanXuat) {
			...
			for (NhaCungCapDTO ncc : danhSachCanXuat) {
				...
				if (dsPhieuNhap != null && !dsPhieuNhap.isEmpty()) {
					for (PhieuNhapDTO pn : dsPhieuNhap) {
						...
						dataRow.createCell(3).setCellValue(pn.getNgayNhap() != null ? pn.getNgayNhap() : "");
						dataRow.createCell(4).setCellValue(df.format(pn.getTongTien()));
						dataRow.createCell(5).setCellValue(pn.getTenNhanVien() != null ? pn.getTenNhanVien() : "");
				...
				if (thongKe != null && !thongKe.isEmpty()) {
					for (Map.Entry<String, Object[]> entry : thongKe.entrySet()) {
						...
						dataRow.createCell(4)
								.setCellValue(data[1] != null ? data[1].toString() : ""); // Loại
