...
public class TraCuuDonTraHang_GUI extends JPanel implements ActionListener {
	...
	private final ClientService chiTietPhieuTraDAO;

	// DATA
	private List<PhieuTraDTO> allPhieuTra = new ArrayList<>();
	...
	public TraCuuDonTraHang_GUI() {
		...
		chiTietPhieuTraDAO = new ClientService();
		initialize();
	}
	...
	private void xemPhieuTra(String maPT) {
		PhieuTraDTO pt = phieuTraDAO.getPhieuTraByCode(maPT);
		if (pt != null) {
			List<ChiTietPhieuTraDTO> dsCT = chiTietPhieuTraDAO.getChiTietPhieuTraByMa(maPT);
	...
	private void taiDanhSachPhieuTra() {
		allPhieuTra = phieuTraDAO.getAllPhieuTra();
	}
	...
	private void xuLyTimKiem() {
		...
		List<PhieuTraDTO> dsFinal = new ArrayList<>();
		...
		for (PhieuTraDTO pt : ketQua) {
			// Lọc Ngày
			LocalDate ngayLap = parseNgayLap(pt.getNgayLap());
	...
	// LOAD BẢNG PHIẾU TRẢ
	// ==============================================================================
	private void loadTablePhieuTra(List<PhieuTraDTO> ds) {
	...
	private void loadChiTietTuDongChon() {
		...
		List<ChiTietPhieuTraDTO> ds = chiTietPhieuTraDAO.getChiTietPhieuTraByMa(maPT);

		modelChiTiet.setRowCount(0);
		int stt = 1;

		for (ChiTietPhieuTraDTO ct : ds) {
	...
	private LocalDate parseNgayLap(String s) {
		...
		try { return LocalDate.parse(s, dtf); } catch (Exception ignored) { }
