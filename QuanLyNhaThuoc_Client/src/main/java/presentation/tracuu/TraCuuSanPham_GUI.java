...
public class TraCuuSanPham_GUI extends JPanel implements ActionListener {
    ...
    // Cache Data
    private List<SanPhamDTO> dsSanPhamHienTai;
    ...
    private void taoPhanHeader() {
        ...
        for (String loai : layDanhSachLoaiSanPham()) {
            cbLoai.addItem(loai);
        }
    ...
    private void loadChiTietTuDongChon() {
        ...
        if (row >= 0) {
            ...
            SanPhamDTO spChon = dsSanPhamHienTai.stream()
    ...
    private boolean validateTimKiem() {
        ...
        if (dsSanPhamHienTai != null && !dsSanPhamHienTai.isEmpty()) {
            for (SanPhamDTO sp : dsSanPhamHienTai) {
    ...
    private void xuLyTimKiem() {
        ...
        List<SanPhamDTO> ketQuaTimKiem = tuKhoa.isEmpty() ? layDanhSachSanPham() : timKiemSanPham(tuKhoa);
        ...
        List<SanPhamDTO> ketQuaCuoiCung = ketQuaTimKiem.stream().filter(sp -> {
            boolean passLoai = "Tất cả".equals(loaiChon) ||
                    (sp.getLoaiSanPham() != null && sp.getLoaiSanPham().equals(loaiChon));
    ...
    private void renderBangSanPham(List<SanPhamDTO> list) {
        modelSanPham.setRowCount(0);
        int stt = 1;
        for (SanPhamDTO sp : list) {
            String trangThaiText = sp.isHoatDong() ? "Đang bán" : "Ngừng kinh doanh";
            String loaiText = sp.getLoaiSanPham() != null ? sp.getLoaiSanPham() : "";
            String duongDungText = sp.getDuongDung() != null ? sp.getDuongDung() : "Không xác định";
    ...
    private List<String> layDanhSachLoaiSanPham() {
        List<String> result = new ArrayList<>();
        for (SanPhamDTO sp : layDanhSachSanPham()) {
            String loai = sp.getLoaiSanPham();
            if (loai != null && !loai.isBlank() && !result.contains(loai)) {
                result.add(loai);
            }
        }
        return result;
    ...
    private List<SanPhamDTO> layDanhSachSanPham() {
        try {
            return new ArrayList<>(svc.getAllSanPhamDTO());
        } catch (Exception ex) {
            return new ArrayList<>();
        }
    }

    private List<SanPhamDTO> timKiemSanPham(String tuKhoa) {
        try {
            return new ArrayList<>(svc.searchProductsDTO(tuKhoa));
        } catch (Exception ex) {
            return new ArrayList<>();
        }
    }

    private List<LoSanPhamDTO> layDanhSachLoTheoSanPham(String maSanPham) {
        try {
            return new ArrayList<>(svc.getLotsByProductDTO(maSanPham));
        } catch (Exception ex) {
            return new ArrayList<>();
        }
    }

    private List<QuyCachDongGoiDTO> layDanhSachQuyCachTheoSanPham(String maSanPham) {
        try {
            return new ArrayList<>(svc.getPackagingRulesByProductDTO(maSanPham));
        } catch (Exception ex) {
            return new ArrayList<>();
        }
    }

    private void loadChiTietSanPham(SanPhamDTO sp) {
        modelLoSanPham.setRowCount(0);
        modelQuyCach.setRowCount(0);
        ...
        List<LoSanPhamDTO> listLo = layDanhSachLoTheoSanPham(maSP);
        int sttLo = 1;
        for (LoSanPhamDTO lo : listLo) {
            modelLoSanPham.addRow(new Object[] {
                    sttLo++,
                    lo.getMaLo(),
                    lo.getHanSuDung() != null ? lo.getHanSuDung() : "",
                    lo.getSoLuongTon()
            });
        }

        List<QuyCachDongGoiDTO> listQC = layDanhSachQuyCachTheoSanPham(maSP);
        int sttQC = 1;
        for (QuyCachDongGoiDTO qc : listQC) {
    ...
    private void xuatExcel() {
        ...
        List<SanPhamDTO> danhSachCanXuat;
        String tenFile;
        ...
        if (coChonDong) {
            ...
            for (int row : selectedRows) {
                String maSP = tblSanPham.getValueAt(row, 1).toString();
                SanPhamDTO sp = dsSanPhamHienTai.stream()
                        .filter(s -> s.getMaSanPham().equals(maSP))
                        .findFirst()
                        .orElse(null);
                if (sp != null) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            int rowIdx = 1;
            for (SanPhamDTO sp : danhSachCanXuat) {
            ...
            for (SanPhamDTO sp : danhSachCanXuat) {
                ...
                for (LoSanPhamDTO lo : listLo) {
                    ...
                    dataRow.createCell(3).setCellValue(lo.getHanSuDung() != null ? lo.getHanSuDung() : "");
            ...
            for (SanPhamDTO sp : danhSachCanXuat) {
                ...
                for (QuyCachDongGoiDTO qc : listQC) {
                    ...
                    String tenDVT = qc.getDonViTinh() != null ? qc.getDonViTinh().getTenDonViTinh() : "N/A";
                    double giaBanQuyCach = giaBan * qc.getHeSoQuyDoi() * (1 - qc.getTiLeGiam());
                    String loaiDVT = qc.isDonViGoc() ? "Đơn vị gốc" : "Quy đổi";
                    String tiLeGiamText = (int) (qc.getTiLeGiam() * 100) + "%";
