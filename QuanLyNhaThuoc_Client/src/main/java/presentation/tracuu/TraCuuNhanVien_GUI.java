...
public class TraCuuNhanVien_GUI extends JPanel {
	...
	private final ClientService nvDAO;
	private final ClientService lichSuDAO;

	private List<NhanVienDTO> danhSachGoc = new ArrayList<>();
	...
	public TraCuuNhanVien_GUI() {
		...
		nvDAO = new ClientService();
		lichSuDAO = new ClientService();
		initialize();
	}
	...
	private void xuLyLamMoi() {
		...
		// Socket service fetches fresh data on each request.
		taiDanhSachNhanVien();
	...
	private void taiDanhSachNhanVien() {
		danhSachGoc = layDanhSachNhanVien();
	}

	private List<NhanVienDTO> layDanhSachNhanVien() {
		try {
			return new ArrayList<>(nvDAO.getAllNhanVienDTO());
		} catch (Exception ex) {
			return new ArrayList<>();
		}
	}

	private void loadTableNhanVien(List<NhanVienDTO> ds) {
		modelNhanVien.setRowCount(0);
		int stt = 1;

		for (NhanVienDTO nv : ds) {
			modelNhanVien.addRow(new Object[] { stt++, nv.getMaNhanVien(), nv.getTenNhanVien(),
					nv.getGioiTinh() != null ? nv.getGioiTinh() : "", nv.getNgaySinh() != null ? nv.getNgaySinh() : "",
					nv.getSoDienThoai(), nv.getVaiTro() != null ? nv.getVaiTro() : "", nv.getCaLam() != null ? nv.getCaLam() : "",
	...
	private void xuLyTimKiem() {
		...
		// 2. Nếu có keyword cụ thể → Tìm kiếm trong cache (tối ưu)
			...
			for (NhanVienDTO nv : danhSachGoc) {
		...
		if (!"Tất cả".equals(cv)) {
			ketQua.removeIf(nv -> !cv.equals(nv.getVaiTro()));
		}

		// --- Áp dụng bộ lọc: ca làm ---
		if (!"Tất cả".equals(ca)) {
			ketQua.removeIf(nv -> !ca.equals(nv.getCaLam()));
		}

		// --- Áp dụng bộ lọc: trạng thái ---
		if (!"Tất cả".equals(tt)) {
			ketQua.removeIf(nv -> !tt.equals(nv.getTrangThai()));
		}
	...
	private void taiLichSuBanHang() {
		...
		List<HoaDonDTO> ds = lichSuDAO.getLichSuBanTheoNhanVien(maNV, null, null);
		...
		for (HoaDonDTO hd : ds) {
			modelBan.addRow(
					new Object[] { stt++, hd.getMaHoaDon(), hd.getNgayLap() != null ? hd.getNgayLap() : "",
							hd.getTenKhachHang() != null ? hd.getTenKhachHang() : "",
							df.format(hd.getThanhToan()) });
		}
	}

	private List<PhieuTraDTO> layLichSuTraTheoNhanVien(String maNV) {
		try {
			List<?> ds = lichSuDAO.getLichSuTraTheoNhanVien(maNV, null, null);
			List<PhieuTraDTO> result = new ArrayList<>();
			if (ds != null) {
				for (Object o : ds) {
					if (o instanceof PhieuTraDTO) {
						result.add((PhieuTraDTO) o);
					}
				}
			}
			return result;
	...
	private List<PhieuHuyDTO> layLichSuHuyTheoNhanVien(String maNV) {
		try {
			List<?> ds = lichSuDAO.getLichSuHuyTheoNhanVien(maNV, null, null);
			List<PhieuHuyDTO> result = new ArrayList<>();
			if (ds != null) {
				for (Object o : ds) {
					if (o instanceof PhieuHuyDTO) {
						result.add((PhieuHuyDTO) o);
					}
				}
			}
			return result;
	...
	private void taiLichSuTraHang() {
		...
		List<PhieuTraDTO> ds = layLichSuTraTheoNhanVien(maNV);
	...
	private void taiLichSuHuyHang() {
		...
		List<PhieuHuyDTO> ds = layLichSuHuyTheoNhanVien(maNV);

		modelHuy.setRowCount(0);
		int stt = 1;

		for (PhieuHuyDTO ph : ds) {
			modelHuy.addRow(new Object[] { stt++, ph.getMaPhieuHuy(),
					ph.getNgayLapPhieu() != null ? ph.getNgayLapPhieu().format(dtf) : "",
					ph.getNhanVien() != null ? ph.getNhanVien().getTenNhanVien() : "", ph.isTrangThai() ? "Đã duyệt" : "Chờ duyệt",
	...
	private void xuatExcelDayDu() {
		...
		List<HoaDonDTO> dsBanXuat = new ArrayList<>();
		List<PhieuTraDTO> dsTraXuat = new ArrayList<>();
		List<PhieuHuyDTO> dsHuyXuat = new ArrayList<>();

		for (String maNV : dsMaNVCanXuat) {
			dsBanXuat.addAll(lichSuDAO.getLichSuBanTheoNhanVien(maNV, null, null));
			dsTraXuat.addAll(layLichSuTraTheoNhanVien(maNV));
			dsHuyXuat.addAll(layLichSuHuyTheoNhanVien(maNV));
		}
			...
			if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
				...
				if (!dsBanXuat.isEmpty()) {
					...
					for (int row = 0; row < dsBanXuat.size(); row++) {
						HoaDonDTO hd = dsBanXuat.get(row);
						...
						dataRow.createCell(2).setCellValue(hd.getNgayLap() != null ? hd.getNgayLap() : "");
						dataRow.createCell(3).setCellValue(hd.getTenNhanVien() != null ? hd.getTenNhanVien() : "");
						dataRow.createCell(4).setCellValue(hd.getTenKhachHang() != null ? hd.getTenKhachHang() : "");
						dataRow.createCell(5).setCellValue(df.format(hd.getThanhToan()));
					}
				...
				if (!dsTraXuat.isEmpty()) {
					...
					for (int row = 0; row < dsTraXuat.size(); row++) {
						PhieuTraDTO pt = dsTraXuat.get(row);
						Row dataRow = sheetTra.createRow(row + 5);
						dataRow.createCell(0).setCellValue(stt++);
						dataRow.createCell(1).setCellValue(pt.getMaPhieuTra());
						dataRow.createCell(2).setCellValue(pt.getNgayLap() != null ? pt.getNgayLap() : "");
						dataRow.createCell(3)
								.setCellValue(pt.getTenNhanVien() != null ? pt.getTenNhanVien() : "");
						dataRow.createCell(4)
								.setCellValue(pt.getTenKhachHang() != null ? pt.getTenKhachHang() : "");
				...
				if (!dsHuyXuat.isEmpty()) {
					...
					for (int row = 0; row < dsHuyXuat.size(); row++) {
						PhieuHuyDTO ph = dsHuyXuat.get(row);
						...
						dataRow.createCell(4).setCellValue(ph.isTrangThai() ? "Đã duyệt" : "Chờ duyệt");
