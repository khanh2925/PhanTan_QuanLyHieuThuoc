...
public class DataSeeder {
    ...
    public static void seed() {
        ...
        try {
            ...
            if (count > 0) {
                tx.begin();
                ensureDefaultBangGia(em);
                tx.commit();
                System.out.println("[DataSeeder] Dữ liệu đã tồn tại, đã kiểm tra bảng giá mặc định.");
                return;
            }
            ...
            em.persist(new QuyCachDongGoi("QC-000012", dvtHop,  sp6, 20, 0.07, false, true));

            em.persist(new LoSanPham("LO-000007", LocalDate.of(2025, 1, 31), 8, sp4));
            em.persist(new LoSanPham("LO-000008", LocalDate.of(2026, 5, 20), 12, sp2));
            em.persist(new LoSanPham("LO-000009", LocalDate.of(2026, 11, 30), 0, sp5));
            ...
            em.persist(km3); em.persist(km4); em.persist(km5); em.persist(km6);
            em.persist(new ChiTietKhuyenMaiSanPham(sp3, km3));
            em.persist(new ChiTietKhuyenMaiSanPham(sp4, km4));
            em.persist(new ChiTietKhuyenMaiSanPham(sp5, km5));
            em.persist(new ChiTietKhuyenMaiSanPham(sp6, km6));

            HoaDon hd3 = new HoaDon();
            hd3.setMaHoaDon("HD-20240310-0001");
            hd3.setNhanVien(nv2);
            hd3.setKhachHang(kh3);
            hd3.setNgayLap(LocalDate.of(2024, 3, 10));
            hd3.setThuocKeDon(false);
            hd3.getDanhSachChiTiet().add(new ChiTietHoaDon(hd3, lo5, 6.0, dvtVien, sp4.getGiaBan(), null));
            hd3.capNhatDuLieuHoaDon();
            em.persist(hd3);

            HoaDon hd4 = new HoaDon();
            hd4.setMaHoaDon("HD-20250522-0001");
            hd4.setNhanVien(nv3);
            hd4.setKhachHang(kh4);
            hd4.setNgayLap(LocalDate.of(2025, 5, 22));
            hd4.setThuocKeDon(false);
            hd4.getDanhSachChiTiet().add(new ChiTietHoaDon(hd4, lo6, 4.0, dvtVien, sp5.getGiaBan(), null));
            hd4.capNhatDuLieuHoaDon();
            em.persist(hd4);

            PhieuHuy ph2 = new PhieuHuy();
            ph2.setMaPhieuHuy("PH-20250522-0001");
            ph2.setNgayLapPhieu(LocalDate.of(2025, 5, 22));
            ph2.setNhanVien(nv3);
            ...
            PhieuTra pt2 = new PhieuTra();
            pt2.setMaPhieuTra("PT-20250522-0001");
            pt2.setKhachHang(kh2);
            pt2.setNhanVien(nv3);
            ...
            System.out.println("[DataSeeder] ✅ Hoàn tất: " +
                    "6 DVT | 4 NCC | 5 KH | 4 NV | 4 TK | " +
                    "1 BG + 2 CTBG | 6 SP | 12 QC | 10 LO | " +
                    "6 KM + 6 CTKM | 2 PN + 4 CTPN | " +
                    "4 HD + 5 CTHD | 2 PH + 2 CTPH | 2 PT + 2 CTPT");
    ...
    private static void ensureDefaultBangGia(EntityManager em) {
        Long soBangGiaHoatDong = em.createQuery(
                "SELECT COUNT(b) FROM BangGia b WHERE b.hoatDong = true", Long.class)
                .getSingleResult();

        BangGia bangGia = em.find(BangGia.class, "BG-20240101-0001");
        if (bangGia == null) {
            NhanVien nv = em.createQuery("SELECT n FROM NhanVien n ORDER BY n.maNhanVien", NhanVien.class)
                    .setMaxResults(1)
                    .getSingleResult();
            bangGia = new BangGia("BG-20240101-0001", nv, "Bảng giá cơ bản 2024", LocalDate.of(2024, 1, 1), soBangGiaHoatDong == 0);
            em.persist(bangGia);
        } else if (soBangGiaHoatDong == 0) {
            bangGia.setHoatDong(true);
        }

        ensureChiTietBangGia(em, bangGia, 0, 100000, 1.30);
        ensureChiTietBangGia(em, bangGia, 100000, 999999999, 1.25);
        capNhatGiaBanSanPhamThieuGia(em, bangGia);
    }

    private static void ensureChiTietBangGia(EntityManager em, BangGia bangGia, double giaTu, double giaDen, double tiLe) {
        ChiTietBangGia.Id id = new ChiTietBangGia.Id(bangGia.getMaBangGia(), giaTu, giaDen);
        if (em.find(ChiTietBangGia.class, id) == null) {
            em.persist(new ChiTietBangGia(bangGia, giaTu, giaDen, tiLe));
        }
    }

    private static void capNhatGiaBanSanPhamThieuGia(EntityManager em, BangGia bangGia) {
        java.util.List<ChiTietBangGia> chiTietBangGia = em.createQuery(
                "SELECT c FROM ChiTietBangGia c WHERE c.id.maBangGia = :ma", ChiTietBangGia.class)
                .setParameter("ma", bangGia.getMaBangGia())
                .getResultList();
        java.util.List<SanPham> sanPhamThieuGia = em.createQuery(
                "SELECT s FROM SanPham s WHERE s.giaBan <= 0", SanPham.class)
                .getResultList();

        for (SanPham sanPham : sanPhamThieuGia) {
            for (ChiTietBangGia chiTiet : chiTietBangGia) {
                double giaNhap = sanPham.getGiaNhap();
                if (giaNhap >= chiTiet.getGiaTu() && giaNhap <= chiTiet.getGiaDen()) {
                    sanPham.setGiaBan(Math.round(giaNhap * chiTiet.getTiLe()));
                    break;
                }
            }
        }
