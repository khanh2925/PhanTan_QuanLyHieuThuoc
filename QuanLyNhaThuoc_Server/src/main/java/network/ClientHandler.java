...
public class ClientHandler implements Runnable {
    ...
    private final QuyCachDongGoiService quyCachDongGoiService;
    private final ThongKeNhanVienService thongKeNhanVienService;
    ...
    public ClientHandler(Socket socket) {
        ...
        this.phieuTraService = new PhieuTraServiceImpl();
        this.chiTietHoaDonDao = new ChiTietHoaDonDaoImpl();
        this.quyCachDongGoiService = new QuyCachDongGoiServiceImpl();
        this.thongKeNhanVienService = new ThongKeNhanVienServiceImpl();
    }
    ...
    public void run() {
        try (
            ...
            System.err.println("Client error: " + socket.getInetAddress() + " - " + ex.getMessage());
            ex.printStackTrace();
        }
    }
        ...
        try {
            switch (cmd) {
                ...
                case TAIKHOAN_TIM_QUEN_MAT_KHAU -> {
                    Object[] params = (Object[]) data;
                    String maTaiKhoan = taiKhoanService.timTaiKhoanQuenMatKhau(
                ...
                case TAIKHOAN_DAT_LAI_MAT_KHAU -> {
                    Object[] params = (Object[]) data;
                    boolean ok = taiKhoanService.capNhatMatKhau((String) params[0], (String) params[1]);
                    response.setSuccess(ok);
                ...
                case PHIEUHUY_LAY_THEO_MA -> {
                    if (data instanceof Object[] params && params.length > 0 && params[0] instanceof String action) {
                        switch (action) {
                            case "LAY_CHI_TIET" -> {
                                String maPhieuHuy = (String) params[1];
                                response.setSuccess(true);
                                response.setData(phieuHuyService.layChiTietTheoMaPhieu(maPhieuHuy).stream()
                                        .map(this::toChiTietPhieuHuyDTO)
                                        .toList());
                            }
                            case "CAP_NHAT_TRANG_THAI_CHI_TIET" -> {
                                String maPhieuHuy = (String) params[1];
                                String maLo = (String) params[2];
                                int trangThai = (Integer) params[3];
                                boolean ok = phieuHuyService.capNhatTrangThaiChiTiet(maPhieuHuy, maLo, trangThai);
                                response.setSuccess(ok);
                                response.setMessage(ok ? "Cập nhật trạng thái chi tiết phiếu hủy thành công"
                                        : "Không thể cập nhật trạng thái chi tiết phiếu hủy");
                            }
                            case "CHECK_TRANG_THAI" -> {
                                boolean ok = phieuHuyService.kiemTraTrangThai((String) params[1]);
                                response.setSuccess(true);
                                response.setData(ok);
                            }
                            case "CAP_NHAT_TRANG_THAI_PHIEU" -> {
                                boolean ok = phieuHuyService.capNhatTrangThaiPhieuHuy((String) params[1]);
                                response.setSuccess(ok);
                                response.setMessage(ok ? "Cập nhật trạng thái phiếu hủy thành công"
                                        : "Phiếu hủy vẫn còn chi tiết chờ duyệt");
                            }
                            default -> {
                                response.setSuccess(false);
                                response.setMessage("Hành động phiếu hủy không được hỗ trợ: " + action);
                            }
                        }
                    } else {
                ...
                case PHIEUHUY_THEM -> {
                    PhieuHuy ph = data instanceof PhieuHuyDTO dto ? toPhieuHuyEntity(dto) : (PhieuHuy) data;
                    boolean ok = phieuHuyService.themPhieuHuy(ph);
                    response.setSuccess(ok);
                ...
                case QUYCACH_LAY_THEO_SAN_PHAM -> {
                    response.setSuccess(true);
                    response.setData(quyCachDongGoiService.layDanhSachQuyCachTheoSanPham((String) data).stream()
                ...
                case PHIEUTRA_LAY_CHI_TIET -> {
                    response.setSuccess(true);
                    response.setData(phieuTraService.timKiemChiTietBangMaPhieuTra((String) data));
                }
                case PHIEUTRA_LAY_THEO_SDT -> {
                    response.setSuccess(true);
                    response.setData(phieuTraService.timPhieuTraTheoSoDienThoai((String) data));
                }
                case PHIEUTRA_LAY_THEO_KEYWORD -> {
                    response.setSuccess(true);
                    response.setData(phieuTraService.timPhieuTraTheoKeyword((String) data));
                }
                case PHIEUTRA_THEM -> {
                    Object[] params = (Object[]) data;
                    PhieuTra pt;
                    List<ChiTietPhieuTra> dsChiTiet;
                    if (params[0] instanceof PhieuTraDTO dto) {
                        @SuppressWarnings("unchecked")
                        List<ChiTietPhieuTraDTO> chiTietDtos = (List<ChiTietPhieuTraDTO>) params[1];
                        pt = toPhieuTraEntity(dto);
                        dsChiTiet = toChiTietPhieuTraEntityList(chiTietDtos, pt);
                    } else {
                        pt = (PhieuTra) params[0];
                        @SuppressWarnings("unchecked")
                        List<ChiTietPhieuTra> entities = (List<ChiTietPhieuTra>) params[1];
                        dsChiTiet = entities;
                    }
                ...
                case PHIEUTRA_CAP_NHAT_TRANG_THAI_GIAO_DICH -> {
                    Object[] params = (Object[]) data;
                    String kq = phieuTraService.capNhatTrangThaiGiaoDich(
                ...
                case PHIEUTRA_TAO_MA -> {
                    response.setSuccess(true);
                    response.setData(phieuTraService.taoMaPhieuTra());
                }
                case PHIEUTRA_DEM_HOM_NAY_NV -> {
                    response.setSuccess(true);
                    response.setData(phieuTraService.demSoPhieuTraHomNayCuaNhanVien((String) data));
                }
                case PHIEUTRA_TONG_SO_LUONG_DA_TRA -> {
                    Object[] params = (Object[]) data;
                    response.setSuccess(true);
                    response.setData(phieuTraService.tongSoLuongDaTra((String) params[0], (String) params[1]));
                }
                case PHIEUTRA_DA_TRA_LO -> {
                    Object[] params = (Object[]) data;
                    response.setSuccess(true);
                    response.setData(phieuTraService.daTraLoTrongHoaDon((String) params[0], (String) params[1]));
                }

                // ==================== ThongKe NhanVien ====================
                case THONGKE_NHANVIEN_LAY_THONG_KE -> {
                    Object[] params = (Object[]) data;
                    ThongKeNhanVienService.KetQuaThongKe kq = thongKeNhanVienService.getThongKe(
                ...
                case THONGKE_NHANVIEN_LAY_DANH_SACH -> {
                    response.setSuccess(true);
                    response.setData(thongKeNhanVienService.getDanhSachNhanVien());
                }
    ...
    private PhieuHuy toPhieuHuyEntity(PhieuHuyDTO dto) {
        if (dto == null) return null;
        ...
        if (dto.getNhanVien() != null) {
            ...
            nv.setMaNhanVien(dto.getNhanVien().getMaNhanVien());
            nv.setTenNhanVien(dto.getNhanVien().getTenNhanVien());
            ph.setNhanVien(nv);
        }
        if (dto.getChiTietPhieuHuyList() != null) {
            ph.setChiTietPhieuHuyList(dto.getChiTietPhieuHuyList().stream().map(ctDto -> toChiTietPhieuHuyEntity(ctDto, ph)).toList());
        }
        return ph;
    }

    private ChiTietPhieuHuy toChiTietPhieuHuyEntity(ChiTietPhieuHuyDTO dto, PhieuHuy ph) {
        ChiTietPhieuHuy ct = new ChiTietPhieuHuy();
        ct.setPhieuHuy(ph);
        LoSanPham lo = new LoSanPham();
        lo.setMaLo(dto.getMaLo() != null ? dto.getMaLo() : dto.getLoSanPham() != null ? dto.getLoSanPham().getMaLo() : null);
        ct.setLoSanPham(lo);
        ct.setSoLuongHuy(dto.getSoLuongHuy());
        ct.setLyDoChiTiet(dto.getLyDoChiTiet());
        ct.setDonGiaNhap(dto.getDonGiaNhap());
        ct.setTrangThai(dto.getTrangThai());
        if (dto.getDonViTinh() != null) {
            DonViTinh dvt = new DonViTinh();
            dvt.setMaDonViTinh(dto.getDonViTinh().getMaDonViTinh());
            dvt.setTenDonViTinh(dto.getDonViTinh().getTenDonViTinh());
            ct.setDonViTinh(dvt);
        }
        ct.capNhatThanhTien();
        return ct;
    ...
    private NhanVien toNhanVienEntity(Object data) {
        if (data instanceof NhanVien nv) return nv;
    ...
    private PhieuTra toPhieuTraEntity(PhieuTraDTO dto) {
        PhieuTra pt = new PhieuTra();
        pt.setMaPhieuTra(dto.getMaPhieuTra());
        pt.setNgayLap(parseDisplayDate(dto.getNgayLap()));
    ...
    private List<ChiTietPhieuTra> toChiTietPhieuTraEntityList(List<ChiTietPhieuTraDTO> dtos, PhieuTra pt) {
        if (dtos == null) return java.util.Collections.emptyList();
        return dtos.stream().map(dto -> toChiTietPhieuTraEntity(dto, pt)).toList();
    }

    private ChiTietPhieuTra toChiTietPhieuTraEntity(ChiTietPhieuTraDTO dto, PhieuTra pt) {
        ChiTietHoaDon cthd = null;
        if (dto.getMaDonViTinh() != null && !dto.getMaDonViTinh().isBlank()) {
            cthd = chiTietHoaDonDao.timKiemChiTietHoaDonBangMa(
                    dto.getMaHoaDon(), dto.getMaLo(), dto.getMaDonViTinh());
        }
        if (cthd == null) {
            cthd = chiTietHoaDonDao.layDanhSachChiTietTheoMaHD(dto.getMaHoaDon()).stream()
                    .filter(ct -> ct.getId() != null && dto.getMaLo().equals(ct.getId().getMaLo()))
                    .filter(ct -> ct.getDonViTinh() != null
                            && dto.getTenDonViTinh() != null
                            && dto.getTenDonViTinh().equalsIgnoreCase(ct.getDonViTinh().getTenDonViTinh()))
                    .findFirst()
                    .orElse(null);
        }
        if (cthd == null) {
            throw new IllegalArgumentException("Không tìm thấy chi tiết hóa đơn để trả: "
                    + dto.getMaHoaDon() + " - " + dto.getMaLo() + " - " + dto.getTenDonViTinh());
        }
        ...
        ct.setLyDoChiTiet(dto.getLyDoChiTiet());
        ct.setTrangThai(dto.getTrangThai());
        ct.setDonViTinh(cthd.getDonViTinh());
        return ct;
    }

    private LocalDate parseDisplayDate(String date) {
    ...
    private PhieuHuyDTO toPhieuHuyDTO(entity.PhieuHuy entity) {
        ...
        if (entity.getNhanVien() != null) {
            ...
            nv.setSoDienThoai(entity.getNhanVien().getSoDienThoai());
            dto.setNhanVien(nv);
        }
        if (entity.getChiTietPhieuHuyList() != null) {
            dto.setChiTietPhieuHuyList(entity.getChiTietPhieuHuyList().stream().map(this::toChiTietPhieuHuyDTO).toList());
        }
        return dto;
    }

    private ChiTietPhieuHuyDTO toChiTietPhieuHuyDTO(entity.ChiTietPhieuHuy entity) {
        if (entity == null) return null;
        ChiTietPhieuHuyDTO dto = new ChiTietPhieuHuyDTO();
        dto.setMaPhieuHuy(entity.getPhieuHuy() != null ? entity.getPhieuHuy().getMaPhieuHuy() : null);
        if (entity.getLoSanPham() != null) {
            LoSanPhamDTO lo = new LoSanPhamDTO();
            lo.setMaLo(entity.getLoSanPham().getMaLo());
            if (entity.getLoSanPham().getSanPham() != null) {
                lo.setMaSanPham(entity.getLoSanPham().getSanPham().getMaSanPham());
                lo.setTenSanPham(entity.getLoSanPham().getSanPham().getTenSanPham());
            }
            dto.setMaLo(lo.getMaLo());
            dto.setLoSanPham(lo);
        }
        dto.setSoLuongHuy(entity.getSoLuongHuy());
        dto.setLyDoChiTiet(entity.getLyDoChiTiet());
        dto.setDonGiaNhap(entity.getDonGiaNhap());
        dto.setThanhTien(entity.getThanhTien());
        dto.setTrangThai(entity.getTrangThai());
        if (entity.getDonViTinh() != null) {
            DonViTinhDTO dvt = new DonViTinhDTO();
            dvt.setMaDonViTinh(entity.getDonViTinh().getMaDonViTinh());
            dvt.setTenDonViTinh(entity.getDonViTinh().getTenDonViTinh());
