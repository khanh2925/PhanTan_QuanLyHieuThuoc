 ...
 * Không extend AbstractGenericDaoImpl (không có entity tương ứng).
 */
        ...
        try {
            return doInTransaction(em -> {
                ...
                List<Object[]> r = em.createNativeQuery(
                        ...
                        "ORDER BY SUM(cthd.SoLuong * qc.HeSoQuyDoi) DESC LIMIT ?")
        ...
        try {
            return doInTransaction(em -> {
                ...
                String sql =
                        ...
                        "    AND lo.HanSuDung >= CURRENT_DATE AND lo.SoLuongTon > 0 " +
                        "LEFT JOIN ( " +
        ...
        try {
            return doInTransaction(em -> {
                ...
                List<Number> r = em.createNativeQuery(
                        ...
                        "    AND lo.MaSanPham = qc.MaSanPham " +
                        "WHERE lo.MaSanPham = ? " +
                        "    AND hd.NgayLap >= DATE_SUB(CURRENT_DATE, INTERVAL ? DAY)")
        ...
        try {
            return doInTransaction(em -> {
                ...
                List<Object[]> r = em.createNativeQuery(
                        ...
                        "    HAVING COALESCE(SUM(lo.SoLuongTon), 0) <= ? " +
                        ") " +
                        "SELECT ncc.TenNhaCungCap, COUNT(*) " +
                        "FROM SP_TonThap stt " +
        ...
        try {
            return doInTransaction(em -> {
                ...
                String sql =
                        ...
                        "FROM LoSanPham lo " +
                        "INNER JOIN SanPham sp ON lo.MaSanPham = sp.MaSanPham " +
                        "WHERE lo.HanSuDung >= CURRENT_DATE " +
                        "    AND lo.HanSuDung <= DATE_ADD(CURRENT_DATE, INTERVAL ? DAY) " +
        ...
        try {
            return doInTransaction(em -> {
                ...
                List<Number> r = em.createNativeQuery(
                        ...
                        "INNER JOIN QuyCachDongGoi qc ON cthd.MaDonViTinh = qc.MaDonViTinh " +
                        "    AND lo.MaSanPham = qc.MaSanPham " +
                        "WHERE cthd.MaLo = ? AND hd.NgayLap >= DATE_SUB(CURRENT_DATE, INTERVAL ? DAY)")
    ...
    public List<BanGhiThongKe> getDoanhThuTheoNgay(Date tuNgay, Date denNgay, String loaiSP, String maKM) {
        try {
            return doInTransaction(em -> {
                ...
                String sql = "SELECT DATE_FORMAT(hd.NgayLap, '%d/%m/%Y'), SUM(ct.ThanhTien), COUNT(DISTINCT hd.MaHoaDon) " +
