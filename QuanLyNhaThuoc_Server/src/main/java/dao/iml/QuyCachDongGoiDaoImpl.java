...
public class QuyCachDongGoiDaoImpl
        extends AbstractGenericDaoImpl<QuyCachDongGoi, String>
        implements QuyCachDongGoiDao {

    public QuyCachDongGoiDaoImpl() {
    ...
    public String taoMaQuyCach() {
        return doInTransaction(em -> {
            ...
            List<String> result = em.createNativeQuery(
                    "SELECT MaQuyCach FROM QuyCachDongGoi WHERE MaQuyCach LIKE 'QC-%' ORDER BY MaQuyCach DESC LIMIT 1")
                    .getResultList();
            if (!result.isEmpty()) {
