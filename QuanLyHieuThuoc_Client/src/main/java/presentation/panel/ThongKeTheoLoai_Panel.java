...
public class ThongKeTheoLoai_Panel extends JPanel {
    ...
    private void loadDuLieu() {
        ...
        Map<String, Double> doanhThuNamTruoc;
        double tongDoanhThuNamTruoc;
        try {
            danhSach = clientService.getThongKeTheoLoaiSanPham(nam);
            doanhThuNamTruoc = clientService.getDoanhThuNamTruocTheoLoai(nam);
            tongDoanhThuNamTruoc = clientService.getTongDoanhThuTheoNam(nam - 1);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Lỗi tải thống kê theo loại: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
