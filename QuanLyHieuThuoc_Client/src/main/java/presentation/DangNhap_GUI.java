...
public class DangNhap_GUI extends JFrame {
	...
	private JPanel createLeftPanel() {
		...
		ImagePanel pnlCenterBackground = new ImagePanel(loadImageOrNull("/resources/images/Login.png"));
		if (pnlCenterBackground.getImage() == null) {
			pnlCenterBackground.setBackground(new Color(0xB2EBF2));
		}
	...
	private JPanel createLoginFormPanel() {
		...
		Image logoImage = loadScaledImage("/resources/images/Logo.png", 250, 250);
		JLabel lblLogo = logoImage != null ? new JLabel(new ImageIcon(logoImage)) : new JLabel("HOA AN");
		if (logoImage == null) {
			lblLogo.setHorizontalAlignment(SwingConstants.CENTER);
			lblLogo.setFont(new Font("Arial", Font.BOLD, 40));
			lblLogo.setForeground(new Color(0x006064));
		}
		...
		// === 1. Khởi tạo Icon ===
		ImageIcon iconOpen = loadScaledIcon("/resources/images/eye_open.png", 25, 25);
		ImageIcon iconClose = loadScaledIcon("/resources/images/eye_close.png", 25, 25);

		// === 2. Tạo nút toggle và cấu hình giao diện ===
		JButton btnTogglePassword = new JButton(iconOpen != null ? iconOpen : new ImageIcon());
	...
	private Image loadImageOrNull(String path) {
		...
		if (location == null) {
			System.err.println("Khong tim thay resource: " + path);
			return null;
		}
	...
	private Image loadScaledImage(String path, int width, int height) {
		Image image = loadImageOrNull(path);
		return image == null ? null : image.getScaledInstance(width, height, Image.SCALE_SMOOTH);
	}
