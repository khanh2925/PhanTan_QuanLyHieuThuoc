...
public class QLTraHang_GUI extends JPanel implements ActionListener, MouseListener {
	...
	private void loadDataTablePT() {
		...
		dsPhieuTra = new ArrayList<>(svc.getAllPhieuTra().stream()
				.filter(PhieuTra.class::isInstance)
				.map(PhieuTra.class::cast)
				.toList());
	...
	private void loadTableCTPT() {
		...
		dsCTPhieuTra = new ArrayList<>(svc.getChiTietPhieuTraByMa(maPT));
	...
	private void xemPhieuTra(String maPT) {
		PhieuTra pt = (PhieuTra) svc.getPhieuTraByCode(maPT);
		if (pt != null) {
			List<ChiTietPhieuTra> dsCT = svc.getChiTietPhieuTraByMa(maPT);
	...
	private void HuyHang() {
		...
		String kq = svc.capNhatTrangThaiPhieuTraGiaoDich(maPT, maHD, maLo, maDVT, nv, 2);
	...
	private void NhapKho() {
		...
		LoSanPham loSanPham = (LoSanPham) svc.getLotByCode(maLo);
		...
		if (loSanPham != null) {
			...
			QuyCachDongGoi qcGoc = ((List<QuyCachDongGoi>) svc.getPackagingRulesByProduct(loSanPham.getSanPham().getMaSanPham()))
					.stream().filter(QuyCachDongGoi::isDonViGoc).findFirst().orElse(null);
		...
		for (ChiTietPhieuTra ct : dsCTPhieuTra) {
			if (ct.getChiTietHoaDon().getHoaDon().getMaHoaDon().equals(maHD)
				...
				if (ct.getDonViTinh() != null && loSanPham != null) {
					QuyCachDongGoi qc = ((List<QuyCachDongGoi>) svc.getPackagingRulesByProduct(loSanPham.getSanPham().getMaSanPham()))
							.stream()
							.filter(item -> item.getDonViTinh() != null
									&& item.getDonViTinh().getMaDonViTinh().equals(ct.getDonViTinh().getMaDonViTinh()))
		...
		String kq = svc.capNhatTrangThaiPhieuTraGiaoDich(maPT, maHD, maLo, maDVT, nv, 1);
	...
	private boolean capNhatTrangThaiPhieuTra(String maPT, boolean daDuyet) {
		return svc.capNhatTrangThaiPhieuTra(maPT, daDuyet);
	}
