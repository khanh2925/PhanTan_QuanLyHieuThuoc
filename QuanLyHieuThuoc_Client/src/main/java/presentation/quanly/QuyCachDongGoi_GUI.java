...
public class QuyCachDongGoi_GUI extends JPanel {
    ...
    private void loadAllDataFromDatabase() {
        dsQuyCach = svc.getAllQuyCachDongGoi();
        dsSanPham = svc.getAllSanPhamEntity();
        dsDonViTinh = svc.getAllDonViTinh();
        loadDataTable();
    }
    ...
    private void handleThem() {
        ...
        if (quyCachMoi != null) {
            if (svc.createQuyCach(quyCachMoi)) {
                loadAllDataFromDatabase();
    ...
    private void handleCapNhat() {
        ...
        if (dialog.isUpdateSuccess()) {
            if (svc.updateQuyCach(qcCanCapNhat)) {
