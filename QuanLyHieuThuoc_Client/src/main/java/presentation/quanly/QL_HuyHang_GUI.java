...
public class QL_HuyHang_GUI extends JPanel implements ActionListener, MouseListener {
	...
	private void loadTableCTPH() {
		...
		try {
			dsCTPhieuHuy = svc.getChiTietPhieuHuy(maPH);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void TuChoiHuy() {
		...
		if (svc.capNhatTrangThaiChiTietPhieuHuy(maPH, maLo, 3)) {
	...
	private void HuyHang() {
		...
		if (svc.capNhatTrangThaiChiTietPhieuHuy(maPH, maLo, 2)) {
			// ✅ Update cột 7 (Trạng thái)
	...
	private void capNhatTrangThaiPhieuSauKhiCapNhatCTPH(String maPhieuHuy) {
		...
		boolean duDuLieuDeDuyet = svc.kiemTraTrangThaiPhieuHuy(maPhieuHuy);
		...
		boolean ok = svc.capNhatTrangThaiPhieuHuy(maPhieuHuy);
		if (!ok) {
	...
	private void xemPhieuHuy(String maPH) {
		PhieuHuy ph = svc.getPhieuHuyByCode(maPH);
		if (ph != null) {
			List<ChiTietPhieuHuy> dsCT = svc.getChiTietPhieuHuy(maPH);
